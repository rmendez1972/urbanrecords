...
if (isset($_POST['ingresar']) && $_POST['ingresar']=='OK'){
		...
		$consulta= "UPDATE seguimiento SET fecha='".$_POST['fecha']."', observaciones='".$_POST['observaciones']."' WHERE anio=".$_POST['anio']." AND id_solicitud=".$_POST['id_solicitud']." AND id_seguimiento=".$_POST['etapa'];
		...
		if ( $_POST['etapa']==1 && isset($_POST['id_revisor']) && ($_POST['id_revisor']!="")){
			...
			$consulta= "UPDATE solicitud_revisor SET id_revisor='".$_POST['id_revisor']."',fecha_ingreso='".$_POST['fecha']."' WHERE anio=".$_POST['anio']." AND id_solicitud=".$_POST['id_solicitud'];
		...
		if ($_POST['etapa']==4){
			...
			for($i=0;$array_constancia[$i];$i++){
				// Insertar datos personales en tabla aspirantes
...
//-->
 ...
 <table width="90%" class="tabla1" align="center">
  ...
  <tr > 
  	  ...
  	  <td align="center"  class="primeralinea"> Modificar Seguimiento Solicitud</td>
  ...
  <tr>	<td align=center ><br>
		...
		<table width="95%" class="tabla2">
          ...
          <tr > 
            ...
            <td class="segundalinea" ><input name="fecha" type="text" style="vertical-align:middle" size="15" maxlength="10" value="<?PHP  echo $seguimiento['fecha'] ?>" readonly />
          ...
          <tr >
            ...
            <?PHP 
				...
				while($constancias= mysql_fetch_array($resultado)){
					echo '<option value"'.$constancias['numero'].'" >'.$constancias['numero'].'</option>';
				}
				mysql_free_result($resultado);
			?>
            </select>
          ...
          <?PHP 
				...
				$resultado=mysql_query($consulta,$conexion) or die ("La consulta:<br>".$consulta."<br>No se realiz&oacute; correctamente.<br>Copie la sentencia y consulte con el administrador del sistema");
				$pagos= mysql_fetch_array($resultado);
				mysql_free_result($resultado);
			?>

          <tr >
            <td class="segundalinea">Sanci&oacute;n</td>
            <td class="segundalinea" ><input name="sancion" style="text-align:right" type="text" size="15" maxlength="10" value="<?PHP  echo $pagos['sancion'] ?>" onblur="calculo();" />            </td>
          </tr>
          <tr >
            <td class="segundalinea">Derechos</td>
            <td class="segundalinea" ><input name="derechos" style="text-align:right" type="text" size="15" maxlength="10" value="<?PHP  echo $pagos['derechos'] ?>" onblur="calculo();"/>            </td>
          </tr>
          <tr >
            <td class="segundalinea">Monto a pagar</td>
            <td class="segundalinea" ><input name="monto" style="text-align:right" type="text" size="15" maxlength="10" value="<?PHP  echo ($pagos['sancion'] + $pagos['derechos']) ?>" readonly="readonly" />
          ...
          <tr >
            ...
            <td class="segundalinea" ><input name="revisor"  type="text" style="vertical-align:middle" value="<?PHP  echo $usuario['nombre'] ?>" size="70" readonly=""> <input name="id_revisor" type="hidden" value="<?PHP  echo $usuario['id_revisor'] ?>" />
