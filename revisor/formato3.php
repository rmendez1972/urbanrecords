...
if (isset($_POST['accion']) && $_POST['accion']=='asignar'){
		...
		$consulta= "DELETE FROM lotes_resulta2 WHERE anio=".$_GET['anio']." AND id_solicitud=".$_GET['id_solicitud'];
	//			echo $consulta."<br>";
		...
		$consulta= "insert into lotes_resulta2 values (".$_GET['anio'].",".$_GET['id_solicitud'].",'".$_POST['resultante']."',NOW())";
	//			echo $consulta."<br>";
		mysql_query($consulta,$conexion) or die ("La consulta:<br>".$consulta."<br>No se realiz&oacute; correctamente.<br>Copie la sentencia y consulte con el administrador del sistema");
...
function actualizar(){
		cad=document.formulario.resultante.value;
		nombrecampo="Resultante. Asignar el nombre del lote Resultante";
		if (cad==""){
			alert("Error en "+ nombrecampo);
		}else{	
			document.forms.formulario.accion.value="asignar";
			document.forms.formulario.submit( );
		}
}
function finalizar(){
...
//-->
 ...
 <table width="80%" class="tablaDatos" align="center">
   <tr >
     <td height="28" colspan="4" align="center"  class="primeralinea">Relotificaci&oacute;n (Fusi&oacute;n)</td>
     <td rowspan="2" align="center"  class="primeralinea">Superficie Total</td>
   </tr>
   <tr class="tituTab">
     <td height="28" colspan="2" align="center"  class="primeralinea">Lotes</td>
     <td align="center"  class="primeralinea">Área en m<sup>2</sup></td>
     <td align="center"  class="primeralinea">Resultante</td>
   </tr>
   <?php
   ...
   <tr >
     ...
     <td rowspan="<?php echo $registros; ?>" align="center"  class="segundalinea"><?PHP  $consulta= "select lote from lotes_resulta2 WHERE anio=".$_GET['anio']." AND id_solicitud=".$_GET['id_solicitud'];
       ...
       <input type="text" name="resultante" id="resultante" maxlength="25" <?php if ($valor!="") echo "value=\"".$valor."\"" ?>/>
       <input name="asignar" type="button" class="btnOld" onclick="return actualizar()" <?php if ($valor=="") echo "value=\"Asignar\""; else echo "value=\"Modificar\""; ?> /></td>
